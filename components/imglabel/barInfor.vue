<template>
  <div class="demo">
    ...
    <el-row>
      ...
      <el-button type="info"   :disabled="true" @click="corp(true)" >裁剪</el-button>
      <el-button type="info"  :disabled="true" @click="cutOut(true)">抠图</el-button>
      <el-button type="primary" @click="moveSvg()">{{!dragSvg?'开启移动':'关闭移动'}}</el-button>
...
export default {
  ...
  methods: {
    ...
    switchGrap(drag){
      drag?this.$refs.borderBase.rectangularLabel():this.$refs.borderBase.irregularLabeling();
    },
    corp(){
      this.delLabel()
    ...
    moveSvg(){
      ...
      this.dragSvg?this.initEventEmitter.dispatch('OPEN_DRAG--svg'):this.initEventEmitter.dispatch('CLOSE_DRAG--svg')
    }
  }
}
</script>
