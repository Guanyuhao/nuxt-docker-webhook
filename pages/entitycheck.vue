<template>
    <div class='container'>
        ...
        <div class="left-side" :class="{hide:hide,showActive:showActive}" v-show="show">
            ...
            <aside-infor :num='sampleNum'  @taskStand='taskStand' @taskDetail='taskDetail' @goExamp='goExamp' :isSingle="showexam"></aside-infor>
            <div class="aside-item aside-tag">
                ...
                <div class="tag">
                    <img
                    class="icon"
                    src="~/assets/img/kaohe.png"
                    alt=""
                    >
                    <div class="txt">考核标注</div>
                </div>
                </div>
            </div>
            <aside-progress :message="message"></aside-progress>
            <aside-result :list='resultList'></aside-result>
            <aside-task @task="task" @goDetail="goDetail"  :submitTask='submitTask'></aside-task>
        </div>
                ...
                <div class="white-space">
                    ...
                    <div class="bottom-group clearfix">
                        <div class="gruop clearfix">
                            ...
                            <div class="yes" @click="submit" style="40px">
...
export default {
      ...
      methods:{
         ...
         async changSelect(){
           ...
           goDetail(){
                this.$router.push({path:'entityresultview',query:{status:-1,proId:this.$route.query.proId}})
            },
          async search(){
          ...
          //下一页
            nextPage() {
                 if(this.curPagerNo>=this.totalPageNumber){
                    ...
                    this.curPagerNo++;
                    this.$router.push({path:`/entitycheck?page=${this.curPagerNo}`+"&proId="+this.$route.query.proId})
          ...
          //初始化所有标签
            initContent(){
                ...
                if(this.item.isChinese){
                  ...
                  if(this.autoPre){
                      if(!this.bottomList[this.activeIndex].htmlContent&&this.bottomList[this.activeIndex].drop!=1){
                           this.auto()
                      }else{
                          if(this.item.drop!=1){
          ...
          clearAll(){
              for(let i in this.dataArr){
                  ...
                  this.dataArr[i].borderLeft = "";
                    this.dataArr[i].borderTop = ""
                    this.dataArr[i].borderBottom = ""
                    this.dataArr[i].borderRight = ""
                            ...
                            if(str==this.dataArr[i].typeList[j].value){
                                if(this.dataArr[i].typeList[j].back=="background"){
                                    for(let z=this.dataArr[i].typeList[j].index[0];z<=this.dataArr[i].typeList[j].index[1];z++){
                                        ...
                                        this.dataArr[z].borderLeft = "";
                                        this.dataArr[z].borderTop = ""
                                        this.dataArr[z].borderBottom = ""
                                        this.dataArr[z].borderRight = ""
                                    }
                                }else{
                                    for(let z=this.dataArr[i].typeList[j].index[0];z<=this.dataArr[i].typeList[j].index[1];z++){
