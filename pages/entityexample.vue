<template>
    <div class='container'>
        ...
        <div class="left-side" :class="{hide:hide,showActive:showActive}" v-show="show">
            ...
            <aside-progress :message="message" :drop='false'></aside-progress>
...
export default {
      ...
      methods:{
          ...
          async submit(){
              ...
              let { code,msg } = await this.$axios.$post("/label/doLabelExampleOperation", {
         ...
         async dictionClick(){
           ...
           goDetail(){
                this.$router.push({path:'entityresultview',query:{status:-2,proId:this.$route.query.proId}})
            },
         async deleteName(id){
         ...
         async changSelect(){
          ...
          async search(){
            ...
            //提交任务
            async task() {
                let { code, msg } =await this.$axios.$post("/label/commitExampleDataByProId", {
                    proId: this.$route.query.proId
                });
                if (code == 0) {
                    this.$message({
        ...
        async initBottom() {
            let { msg, code, data } = await this.$axios.$post(
                "/label/getExampleListByProId_ner",
                {
            ...
            if(code!=0){
                ...
                for(let i in this.bottomList){
                    if(this.bottomList[i].drop != 0){
                         ...
                         this.bottomList[i].already = false;
                    }else{
