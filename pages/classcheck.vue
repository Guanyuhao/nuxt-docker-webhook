<template>
  <div class='container'>
    ...
    >
      ...
      <aside-infor @taskStand='taskStand' :num='sampleNum' @taskDetail='taskDetail' :isSingle="showexam"  @goExamp='goExamp'></aside-infor>
      <div class="aside-item aside-tag">
        <!-- 任务名称 -->
        <div class="message">
          <div class="title">任务阶段</div>
          <div class="tag">
                    <img class="icon" src="~/assets/img/kaohe.png" alt="">
      ...
      <aside-task @task="task" @goDetail="goDetail"></aside-task>
    </div>
    <div
...
import asideProgress from "../components/classaside/classProcess.vue";
  ...
  methods: {
    ...
    nextPage() {
        ...
        if(this.curPagerNo>=this.totalPageNumber){
          ...
          this.curPagerNo++;
          this.$router.push({path:`/classcheck?page=${this.curPagerNo}`+"&proId="+this.$route.query.proId})
          this.activeIndex = 0;
          this.initBottom();
        }
    },
    ...
    setIndex(index) {
     ...
     goExamp(){
      ...
      this.$router.push({path:"classexampview",query:{proId:this.$route.query.proId}})
    },
    showClick() {
      if (!this.show) {
    ...
    async initBottom() {
      let { msg, code, data, totalPageNumber } = await this.$axios.$post(
