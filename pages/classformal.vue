...
export default {
  ...
  methods: {
    ...
    nextPage() {
      ...
      this.$router.push({
        path:
          `/classformal?page=${this.curPagerNo}` +
          "&proId=" +
    ...
    async initBottom() {
      ...
      if(this.notag){
         let { msg, code, data } = await this.$axios.$get("/push/searchUnLabeledDataList?proId="+this.$route.query.proId)
          if (code != 0) {
            this.$message({
              message: msg,
              type: "warning"
        ...
        this.bottomList =JSON.parse(JSON.stringify(data)) ;
        ...
        let { msg, code, data } = await this.$axios.$post("/push/push", {
        labelType: 2,
      ...
      if (code!=0) {
        ...
        this.activeIndex = 0;
        this.bottomList = [];
        this.bottomList =JSON.parse(JSON.stringify(data.list)) ;
        for (let i in this.bottomList) {
          this.bottomList[i].already = [];
          if (this.bottomList[i].drop != 0) {
            this.bottomList[i].color = "#97183E";
    ...
    async task() {
      let { code, msg } = await this.$axios.$post(
        "/label/updateUserProjectStautaByProIdAndUserId",
        {
          proId: this.$route.query.proId,
          status: 6,
          userId: this.$getCookie("userId")
